#!/bin/bash
# ================================================================
# B2B Design System — Complete Fix + Clean Push
# Usage: bash fix-and-push.sh YOUR_GITHUB_TOKEN
# ================================================================
set -e

TOKEN="${1:-}"
if [ -z "$TOKEN" ]; then
  echo "❌ Cần token: bash fix-and-push.sh ghp_xxx..."
  exit 1
fi
...
REPO="cungnganha/b2b-design-system"

cd "$PROJECT" || err "Không tìm thấy thư mục: $PROJECT"
ok "Project: $PROJECT"

# ── STEP 1: Tạo icons.jsx ─────────────────────────────────────
log "1/10 — Icons system..."
mkdir -p src/components/Icons
cat > src/components/Icons/icons.jsx << 'ICONEOF'
import React from 'react';
const Icon = ({ size=16, color='currentColor', children, title, className, style, ...p }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color}
    strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"
    className={className} style={style} aria-hidden={!title} role={title?'img':undefined} {...p}>
...
export const Phone        = p => <Icon {...p}><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.54 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91A16 16 0 0 0 13 14.82l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></Icon>;
...
export const Icons = { CheckCircle, XCircle, InfoCircle, AlertTriangle, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, ArrowRight, ArrowLeft, ArrowUp, ArrowDown, Plus, Minus, X, Check, Search, Eye, EyeOff, Edit, Trash, Download, Upload, Copy, Link, Refresh, Filter, Calendar, Clock, User, Home, Heart, Star, Bell, Settings, LogOut, Lock, Mail, Phone, Globe, Share, Grid };
export default Icons;
ICONEOF
ok "icons.jsx"

# ── STEP 2: Icons stories ─────────────────────────────────────
log "2/10 — Icons stories..."
...
.ds-btn { display:inline-flex; align-items:center; justify-content:center; font-family:'Lexend',sans-serif; font-weight:500; border:1.5px solid transparent; border-radius:6px; cursor:pointer; white-space:nowrap; outline:none; transition:background 120ms ease,border-color 120ms ease,box-shadow 120ms ease; }
...
.ds-btn--disabled,.ds-btn:disabled { background:#F1F5F9; color:#94A3B8; border-color:transparent; cursor:not-allowed; pointer-events:none; }
...
.ds-input__label { font-family:'Lexend',sans-serif; font-weight:500; color:#0F172A; }
...
.ds-input-field { display:flex; align-items:center; width:100%; background:#fff; border:1.5px solid #E2E8F0; border-radius:6px; transition:border-color 120ms,box-shadow 120ms; box-sizing:border-box; }
...
.ds-input-field:focus-within,.ds-input-field--focus { border-color:#226FF6; box-shadow:0 0 0 3px rgba(34,111,246,0.15); }
...
.ds-input__el { flex:1; min-width:0; border:none; outline:none; background:transparent; font-family:'Lexend',sans-serif; color:#0F172A; }
...
.ds-tooltip-trigger { border:1px solid #E2E8F0; background:#fff; padding:6px 16px; border-radius:6px; cursor:default; font-family:'Lexend',sans-serif; font-size:14px; color:#0F172A; }
.ds-tooltip { position:absolute; z-index:9999; pointer-events:none; padding:7px 10px; border-radius:8px; background:#0F172A; color:#fff; font-family:'Lexend',sans-serif; font-size:12px; font-weight:400; line-height:16px; white-space:nowrap; box-shadow:0 4px 8px rgba(16,24,40,0.12); }
.ds-tooltip::after { content:''; position:absolute; border:5px solid transparent; }
...
# ── STEP 6: Alert ─────────────────────────────────────────────
log "6/10 — Alert (inline SVG icons)..."
...
cat > src/components/Alert/Alert.stories.jsx << 'EOF'
import React from 'react';
...
# ── STEP 7: Fix các component còn lại ─────────────────────────
log "7/10 — Fix Checkbox, Radio, Toggle, Badge, Avatar..."

# Đảm bảo React import không bị duplicate
for f in \
  ...
  src/components/Toggle/Toggle.stories.jsx \
  src/components/Badge/Badge.jsx \
  src/components/Badge/Badge.stories.jsx \
  src/components/Avatar/Avatar.jsx \
  src/components/Avatar/Avatar.stories.jsx; do
  if [ -f "$f" ]; then
    # Remove existing React imports first
    sed -i '' '/^import React/d' "$f" 2>/dev/null || true
    # Add one clean import at top
    echo 'import React from '"'"'react'"'"';' | cat - "$f" > "$f.tmp" && mv "$f.tmp" "$f"
  fi
done
ok "React imports fixed for all components"
...
# ── STEP 9: Xóa token khỏi history & push sạch ───────────────
log "9/10 — Xóa git history có chứa token, tạo history sạch..."

# Cấu hình git user nếu chưa có
git config user.email "design@system.local" 2>/dev/null || true
git config user.name "Design System Bot" 2>/dev/null || true

# Tạo branch mới không có history
git checkout --orphan clean-main

# Add tất cả file hiện tại (không có token trong code)
git add .

# Kiểm tra xem có file nào chứa token không
if grep -r "ghp_" . --include="*.jsx" --include="*.js" --include="*.ts" --include="*.json" 2>/dev/null | grep -v ".git"; then
  echo "⚠️ Tìm thấy token trong file — đang xóa..."
  grep -rl "ghp_" . --include="*.jsx" --include="*.js" --include="*.ts" --include="*.json" 2>/dev/null | xargs sed -i '' 's/ghp_[A-Za-z0-9]*/TOKEN_REMOVED/g' 2>/dev/null || true
  git add .
fi

git commit -m "feat: B2B design system — icons, button, input, tooltip, alert, all components"
...
# ── STEP 10: Push ─────────────────────────────────────────────
log "10/10 — Push lên GitHub..."

# Set remote với token
git remote set-url origin "https://${TOKEN}@github.com/cungnganha/b2b-design-system.git"

# Force push (ghi đè history cũ có token)
git push --force origin main

# Reset remote URL về không có token (bảo mật)
git remote set-url origin "https://github.com/cungnganha/b2b-design-system.git"

echo ""
echo -e "${GREEN}════════════════════════════════════════${NC}"
echo -e "${GREEN}🎉 HOÀN THÀNH! Đã push thành công!${NC}"
echo -e "${GREEN}════════════════════════════════════════${NC}"
echo ""
echo -e "Chờ ~3 phút để GitHub Actions build, rồi mở:"
echo -e "${CYAN}https://cungnganha.github.io/b2b-design-system/${NC}"
echo ""
echo -e "GitHub Actions status:"
echo -e "${CYAN}https://github.com/cungnganha/b2b-design-system/actions${NC}"
